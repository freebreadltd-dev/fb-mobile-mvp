...
export default function HomeScreen() {
  ...
  const loadSellerCatalogs = React.useCallback(async () => {
    if (!session || accountMode !== 'seller') return;
    ...
    try {
      ...
      if (listingsResp.status === false) {
        setCatalogError(listingsResp.message || 'Failed to fetch catalogs');
        setCatalogs([]);
        return;
      }
  ...
  }, [accountMode, api, session]);

  React.useEffect(() => {
    void loadSellerCatalogs();
  }, [loadSellerCatalogs]);
  ...
  const createProductAndListing = React.useCallback(async () => {
    ...
    try {
      ...
      if (listingResp.status === false) {
        setCreateFeedback(listingResp.message || 'Product created, but listing failed.');
        return;
      }
  ...
  }, [api, category, kind, loadSellerCatalogs, name, onHand, priceCents, sellerID, unit]);
    ...
    return (
      <SafeAreaView className="flex-1 bg-white">
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-xl font-bold text-blue-500">Buyer mode</Text>
          <Text className="mt-3 text-center text-base text-gray-500">Coming soon.</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        ...
        {showAddProductForm && (
          <View style={{ gap: 8, borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, padding: 12 }}>
            ...
            <TextInput placeholder="Name" value={name} onChangeText={setName} style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <TextInput placeholder="Category (optional)" value={category} onChangeText={setCategory} style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <TextInput placeholder="Unit (optional)" value={unit} onChangeText={setUnit} style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <TextInput placeholder="Kind (e.g. GENERAL)" value={kind} onChangeText={setKind} style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <TextInput placeholder="Price (cents)" value={priceCents} onChangeText={setPriceCents} keyboardType="numeric" style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <TextInput placeholder="Stock (on_hand)" value={onHand} onChangeText={setOnHand} keyboardType="numeric" style={{ borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }} />
            <Pressable
            ...
            >
              <Text style={{ color: 'white', fontWeight: '700' }}>{creatingProduct ? 'Creating...' : 'Create product'}</Text>
            </Pressable>
            {createFeedback ? <Text style={{ color: '#374151' }}>{createFeedback}</Text> : null}
          </View>
        )}
        ...
        {catalogError ? <Text style={{ color: '#B91C1C' }}>{catalogError}</Text> : null}
        ...
        {catalogs.map((catalog: any) => (
          ...
          >
            ...
            <Text>Price: {catalog.price_cents} {catalog.currency}</Text>
        ...
        {!loadingCatalogs && !catalogError && catalogs.length === 0 ? (
          <Text style={{ color: '#6B7280' }}>No catalogs yet. Add your first product.</Text>
        ) : null}
      </ScrollView>
    </SafeAreaView>
  );
